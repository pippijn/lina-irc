    ...
    TabWidget(parent)
{
    ...
    MessageView* view = openView(d.handler.session()->host(), true);
...
MessageView* SessionTabWidget::openView(const QString& receiver, bool serverView)
{
    MessageView* view = d.views.value(receiver.toLower());
    if (!view)
    {
        view = new MessageView(d.handler.session(), serverView, this);
...
{
    QString channel = QInputDialog::getText(this, tr("Start query"), tr("Target:"));
    if (!channel.isEmpty())
    {
