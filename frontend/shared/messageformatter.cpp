...
MessageFormatter::MessageFormatter(TextBrowser *textBrowser, QObject* parent)
    : QObject(parent)
    , d(textBrowser)
{
}
...
QString MessageFormatter::formatPrivateMessage(IrcPrivateMessage* message) const
{
    foreach (const QString& hilite, d.highlights)
        if (message->message().contains(hilite))
            d.highlight = true;
    QString sender = prettyUser(message->sender());
    QString msg = IrcUtil::messageToHtml(message->message());

    foreach (QString emo, Emoticons::gifs.keys())
    {
        QString const &emotion = Emoticons::gifs[emo];

        QString emoMsg = msg;
        emoMsg.replace(IrcUtil::messageToHtml(emo), "<img src='" + emotion + "'/>");

        if (emoMsg != msg)
        {
            d.textBrowser->addAnimation(QUrl(emotion), ":/resources/emotions/" + emotion + ".gif");
            msg = emoMsg;
        }
    }
