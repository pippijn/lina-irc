...
IrcCommand *CommandParser::parseCommand(const QString& receiver, const QString& text)
{
    ...
    *has_error() = false;
    if (text.startsWith("/"))
    {
        ...
        {
            parseFunctions.insert("AWAY", parseAway);
            parseFunctions.insert("INVITE", parseInvite);
            parseFunctions.insert("KICK", parseKick);
            parseFunctions.insert("ME", parseMe);
            parseFunctions.insert("MODE", parseMode);
            parseFunctions.insert("NAMES", parseNames);
            parseFunctions.insert("NICK", parseNick);
            parseFunctions.insert("NOTICE", parseNotice);
            parseFunctions.insert("PING", parsePing);
            parseFunctions.insert("QUOTE", parseQuote);
            parseFunctions.insert("TIME", parseTime);
            parseFunctions.insert("TOPIC", parseTopic);
            parseFunctions.insert("VERSION", parseVersion);
            parseFunctions.insert("WHOIS", parseWhois);
            parseFunctions.insert("WHOWAS", parseWhowas);
        }
        ...
        if (parseFunc)
        {
            cmd = parseFunc(receiver, words.mid(1));
            if (!cmd)
                *has_error() = true;
        }
    ...
    {
        cmd = IrcCommand::createMessage(receiver, text);
    }
